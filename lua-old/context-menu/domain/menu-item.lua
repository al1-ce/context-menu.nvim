...
function M.trigger_action(self, local_buf_win, context)
    ...
    elseif self.action.type == M.ActionType.sub_cmds then
        ...
        require("context-menu.api").menu_popup_window(
            ...
            context,
            { level = local_buf_win.level + 1, width = local_buf_win.width }
        )
    else
...
function M.format(self, opts)
    ...
    return string.format(
        "%-" .. content_length .. "s %" .. keymap_length .. "s",
...
function M.parse(formatted_str)
    ...
    local line_number_str = "1"
    local content_str = formatted_str:strip_spaces()
    local keymap_str = formatted_str:strip_spaces()
