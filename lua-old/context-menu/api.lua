...
local function generate_window_options(tbl)
    --  'Normal:ContextMenuLineDefault,FloatBorder:ContextMenuLineDefault,FloatTitle:ContextMenuLineDefault,FloatFooter:ContextMenuLineDefault'
    local wopts = ""
...
function M.menu_popup_window(menu_items, context, local_level)
    ...
    local offset = 0
    if local_level.level ~= 1 then offset = local_level.width + 2 end
    ...
    if not context.menu_window then
        win_opts = {
            ...
            col = offset, -- Position the window slightly to the right
            width = width,
            height = height,
            style = "minimal",
            border = vim.g.context_menu_config.ui.border,
            title = "ContextMenu.",
        }
    else
        win_opts = {
            ...
            width = width + 1,
            height = height,
            style = vim.g.context_menu_config.ui.border,
            border = "none",
            title = "ContextMenu. Or why it triggers?",
        }
    end

    local win = vim.api.nvim_open_win(popup_buf, true, win_opts)
    vim.api.nvim_set_option_value('winhighlight', generate_window_options({
        Normal      = "ContextMenuLineDefault",
        FloatBorder = "ContextMenuLineDefault",
        FloatTitle  = "ContextMenuLineDefault",
        FloatFooter = "ContextMenuLineDefault"
    }), { win = win })
    ...
    require("context-menu.hl").create_highlight(menu_items, popup_buf)
end
